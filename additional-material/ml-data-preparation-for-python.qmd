...
The code relies on the following Python libraries. Note that the library tensorflow is only needed if you want to implement the random forests, elastic net, or neural networks, respectively.
...
# Define local paths
# -------------------------
crsp_path = Path("data/crsp_monthly.parquet")
...
# -------------------------
# Load local file if it exists, otherwise download
# -------------------------
if datashare_path.exists():
...
characteristics["month"] = pd.to_datetime(
    characteristics["month"].astype(str), format="%Y%m%d", errors="raise"
)

characteristics["month"] = characteristics["month"].dt.to_period("M").dt.to_timestamp()

cols_remain = ["permno", "month", "sic2"]
characteristics = characteristics.rename(
    columns={
        col: f"characteristic_{col}"
        for col in characteristics.columns
        if col not in cols_remain
    }
)

characteristics = characteristics.dropna(subset=["sic2"])
...
def rank_transform(x):
    ...
    min_rank = 1.0
    max_rank = x.notna().sum()  # equivalent to length(na.omit(x))

    if max_rank == 0:
        return pd.Series(np.nan, index=orig_index)

    denominator = max_rank - min_rank
    scaled = 2 * ((rank_x - min_rank) / denominator - 0.5)

    if denominator == 0:
        scaled = pd.Series(np.nan, index=orig_index)

    return scaled

char_cols = [c for c in characteristics.columns if "characteristic" in c]
...
```{python}
# | label: crsp and macro predictors
# | include: true
# | echo: true
crsp_monthly = pd.read_parquet("data/crsp_monthly.parquet")

crsp_monthly["date"] = pd.to_datetime(crsp_monthly["date"], errors="raise")

crsp_monthly["month"] = crsp_monthly["date"].dt.to_period("M").dt.to_timestamp()

crsp_monthly = crsp_monthly[["month", "permno", "mktcap_lag", "ret"]].rename(
    columns={"ret": "ret_excess"}
)

macro_predictors = pd.read_parquet("data/macro_predictors.parquet")

macro_predictors["date"] = pd.to_datetime(macro_predictors["date"], errors="raise")

macro_predictors["month"] = macro_predictors["date"].dt.to_period("M").dt.to_timestamp()

macro_predictors = macro_predictors[
    ["month", "dp", "ep", "bm", "ntis", "tbl", "tms", "dfy", "svar"]
].rename(columns={"ret_excess": "ret"})

macro_predictors = macro_predictors.rename(
    columns={col: f"macro_{col}" for col in macro_predictors.columns if col != "month"}
)
...
characteristics["macro_intercept"] = 1  
...
# Feature dataframe (drop id + target)
X = characteristics.drop(columns=id_cols + ["ret_excess"])

# -----------------------------
# 2️⃣ Interaction terms
# characteristic_* × macro_*
# -----------------------------
char_cols = [c for c in X.columns if "characteristic" in c]

# subset! CHANGE WHEN DONE
char_cols = char_cols[0:3]
################ REMOVE ABOVE LINE ################
...
# -----------------------------
# 3️⃣ One-hot encode sic2
# -----------------------------
X = pd.get_dummies(X, columns=["sic2"], prefix="sic2")

# -----------------------------
# 4️⃣ Recombine everything
# -----------------------------
characteristics_prepared = pd.concat([characteristics[id_cols], y, X], axis=1)

# -----------------------------
# 5️⃣ Write partitioned by year
# -----------------------------
characteristics_prepared["year"] = characteristics_prepared["month"].dt.year
...
```{python}
# | label: store prepared file
# | include: true
# | echo: true

print("This is not part of my code. Let's see what happens")
```
...
estimation_periods["validation_start"] = (
    estimation_periods["oos_year"] - validation_length
)

estimation_periods["training_start"] = 1957
estimation_periods["training_end"] = estimation_periods["validation_start"] - 1

estimation_periods = estimation_periods[
...
X_train = train.drop(columns=id_cols + ["ret_excess"])
y_train = train["ret_excess"]

X_val = validation.drop(columns=id_cols + ["ret_excess"])
y_val = validation["ret_excess"]
