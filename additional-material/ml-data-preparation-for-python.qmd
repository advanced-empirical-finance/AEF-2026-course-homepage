...
description: "Prepare the Gu, Kelly & Xiu (2020) dataset in Python and set it up for machine-learning models."
jupyter: aef-2026
execute:
    eval: false
---
...
I tune the model and - after selecting the optimal tuning parameter value based on the prediction error in the validation set - fit the penalized regression using the available data (training and validation test set). The process can be faster by distribution the parameter tuning across multiple workers. This is done by setting `n_jobs=-1`. In `sklearn`, `GridSearchCV` handles both the evaluation of multiple hyperparameter combinations and cros-validation in parralel. 
...
:::
To assign portfolios using the predictions, I implement a procedure similar to the `assign_portfolio()` from the tidyfinance package in `R`. I use `pandas.qcut`to divide the predicted reutns into deciles (portfolios 1-10).
...
summary_reset.to_parquet("results/portfolio_summary.parquet", index=False)
```