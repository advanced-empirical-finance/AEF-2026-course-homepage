...
```{python}
#| label: py-load-libraries-lobster
import py7zr
import pandas as pd
import numpy as np
...
```{python}
#| label: py-define-read-archive-csv
...
```{python}
#| label: py-define-process-orderbook
...
You can process the raw data from before by simply calling.
```{python}
#| label: py-process-orderbook
orderbook = process_orderbook(orderbook)
```

The function `compute_depth` below computes the number of traded shares within `bp` basis points of the associated best price, which can be used to measure liquidity.
```{python}
#| label: py-define-compute-depth
...
```{python}
#| label: tbl-py-orderbook-summary
#| tbl-cap: "Five-minute summaries of midquote, spread (bps), trade count, notional volume, and quoted depth for SAP, January 2022."
...
```{python}
#| label: fig-py-orderbook-dynamics
#| fig-cap: "Order book dynamics for SAP: quoted prices at each level (shaded by relative depth) with executed trades overlaid in red."
...
infill_asymptotics <- function(tab, sampling_frequency = 5) {
  tab_xts <- as.xts(tab |> select(price), order.by = tab |> pull(time))
  tab_xts <- aggregatePrice(
    tab_xts,
    alignBy = "minutes",
    alignPeriod = sampling_frequency,
    tz = "GMT",
    marketOpen = "09:30:00"
  )
...
rvs <- simulation |>
  ...
  mutate(
    "1 Second" = map_dbl(data, infill_asymptotics, 1 / 60),
...
rvs |>
  select(-data) |>
  ungroup() |>
  pivot_longer(-iteration, names_to = "Sampling Frequency", values_to = "rv") |>
  mutate(
    `Sampling Frequency` = as.factor(`Sampling Frequency`),
    `Sampling Frequency` = fct_reorder(`Sampling Frequency`, rv, sd)
  ) |>
  ggplot(aes(x = sqrt(rv), fill = `Sampling Frequency`)) +
  geom_density() +
...
Next, we scale the simulation up. The code below simulates 250 paths of the geometric Brownian motion, all with the same starting value. The function illustrates the sampled (log) price paths.
```{python}
#| label: fig-gbm-multiple-paths-py
#| fig-cap: "Two hundred and fifty simulated log price paths of the geometric Brownian motion ($\\sigma = 2.4$, 1000 intraday steps each)."
true_sigma = 2.4
steps = 1000
...
```{python}
#| label: fig-gbm-sample-variation-py
#| fig-cap: "Sample variation of simulated GBM log prices over time; the straight line shows the theoretical $\\sigma^2 t$."
var_df = (
    simulation
    .groupby("step")["price"]
...
```{python}
#| label: fig-infill-asymptotics-py
#| fig-cap: "Sampling distributions of the realized volatility at six different sampling frequencies, illustrating infill asymptotics; the dotted line marks the true volatility $\\sigma$."
#| cache: true
