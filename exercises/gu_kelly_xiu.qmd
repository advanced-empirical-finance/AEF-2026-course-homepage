...
```{r}
#| label: r-download-characteristics
...
characteristics <- characteristics |>
  ...
  mutate(
    date = ymd(date),
    date = floor_date(date, "date")
  ) |>
  rename_with(~ paste0("characteristic_", .), -c(permno, date, sic2))
```
...
```{python}
#| label: py-download-characteristics
...
characteristics = (characteristics
    ...
    .rename(columns={col: 'characteristic_' + col for col in characteristics.columns if col not in ['permno', 'date', 'sic2']})
)    
```
:::
...
characteristics = (characteristics
    ...
    .apply(lambda group: group.apply(lambda x: rank_transform(x) if 'characteristic' in x.name and x.name != 'date' else x))
)
```
...
```{r}
#| label: r-merge-crsp-macro
crsp_monthly <- read_parquet("data-r/crsp_monthly.parquet") |>
  select(date, permno, mktcap_lag, ret_excess)

macro_predictors <- read_parquet("data-r/macro_predictors.parquet") |>
  select(-rp_div) |>
  rename_with(~ paste0("macro_", .), -date)

characteristics <- characteristics |>
  inner_join(crsp_monthly, by = c("date", "permno")) |>
  inner_join(macro_predictors, by = "date") |>
  arrange(permno, date) |>
  select(permno, date, ret_excess, mktcap_lag, sic2, contains("macro"), contains("characteristic"))
```

## Python
```{python}
#| label: py-merge-crsp-macro
crsp_monthly = (pd.read_parquet("data-python/crsp_monthly.parquet")
  [["date", "permno", "mktcap_lag", "ret_excess"]])

macro_predictors = (pd.read_parquet("data-python/macro_predictors.parquet")
  .drop(columns=['rp_div'])
  .rename(columns=lambda x: f'macro_{x}' if x != 'date' else x)
)

characteristics = (
    characteristics
    .merge(crsp_monthly, on=['date', 'permno'], how='inner')
    .merge(macro_predictors, on='date', how='inner')
    .sort_values(by=['permno', 'date'])
    .reset_index(drop=True)  
    [['permno', 'date', 'ret_excess', 'mktcap_lag', 'sic2'] +
