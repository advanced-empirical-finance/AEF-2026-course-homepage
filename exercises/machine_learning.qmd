...
## Python
```{python}
#| label: py-load-libraries-and-data
import pandas as pd
import requests
import numpy as np
...
y = macro_predictors['rp_div'].values
X = macro_predictors.drop(columns=['month', 'rp_div']).values
...
```{r}
#| label: r-define-ridge-regression
...
## Python
```{python}
#| label: py-define-ridge-regression
...
## R
```{r}
#| label: r-define-lasso-regression
...
## Python
```{python}
#| label: py-define-lasso-regression
...
**Solutions:** All solutions are provided in the book chapter *Factor selection via machine learning* ([R](https://www.tidy-finance.org/r/factor-selection-via-machine-learning.html) or [Python](https://www.tidy-finance.org/python/factor-selection-via-machine-learning.html) version).
